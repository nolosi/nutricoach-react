...
export const RecipeService = {
  ...
  getRecipeById: (id: string): Promise<Recipe | null> => {
    return new Promise((resolve) => {
      setTimeout(() => {
        // Zuerst in den Standard-Rezepten (mockRecipes) suchen
        let recipe = mockRecipes.find(r => r.id === id) || null;
        
        // Wenn nicht gefunden, in den benutzerdefinierten Rezepten suchen
        if (!recipe) {
          ...
          recipe = userRecipes.find(r => r.id === id) || null;
        }
